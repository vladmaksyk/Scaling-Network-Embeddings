...
// long proNet::GetSampleNeighbors(long vid, int size) {
...
vector< long > proNet::AmortizedRandomWalk(long start, int walk_times, double jump) {
    list< long > queue_node;  
    list< int > queue_budget;
    long next = start;
    vector< long > walk;
    queue_node.push_back(start);
    queue_budget.push_back(walk_times);
    walk.push_back(next);
    bool flip = false;
    while(!queue_node.empty()){
        long s_vid = queue_node.front(); 
        int budget = queue_budget.front();

        queue_node.pop_front();
        queue_budget.pop_front();

        // cout << "total budget to sahre " << budget << endl;
        int node_degree = vertex[s_vid].branch;
        if(node_degree == 0){
            continue;
        }
        int m = floor((float)budget / (float)node_degree);
        int remainder = budget % node_degree;
        std::vector< long > neighbors;
        // for (int i=0; i < node_degree; i++)
        // {
        //     int seq_id = vertex[s_vid].offset + i;
        //     long neighbor_vid = context[seq_id].vid;
        //     neighbors.push_back(neighbor_vid);
        // }
	...
	//cout << i << " " << start << " " << end << endl;
            // long neighbor_vid = neighbors[i];
            // double randval =  random_gen(0, 1);
            int seq_id = vertex[s_vid].offset + i;
            long neighbor_vid = context[seq_id].vid;
            int budget_for_this_node = m;
            if(remainder > 0){
	    ...
	    if(remainder == 0 && m == 0)
		    break;
            int num_rand_walks_ending_here =  budget_for_this_node * jump;
            for(int i = 0; i < num_rand_walks_ending_here ; ++i){
                    walk.push_back(neighbor_vid);
            }
            int remaining_budget = budget_for_this_node - num_rand_walks_ending_here;
            
            if(remaining_budget > 0){
                if(remaining_budget > 1){
                    queue_node.push_back(neighbor_vid);
                    queue_budget.push_back(remaining_budget);
                }
                else{
                    double randval =  random_gen(0, 1);
                    if(randval < jump){
